...
const MatchCard: React.FC<MatchCardProps> = ({ candidate, matchData, rank }) => {
  ...
  const gradientIndex = (candidate.name.length + candidate.age) % gradients.length;
  ...
  return (
    <div className="glass rounded-[2rem] overflow-hidden hover:translate-y-[-5px] transition-all duration-300 border border-white/5 flex flex-col h-full relative group shadow-xl hover:shadow-purple-500/10">
      ...
      <div className="h-72 overflow-hidden relative bg-zinc-800 group-hover:bg-zinc-700 transition-colors">
        ...
        <div className={`absolute inset-0 bg-gradient-to-br ${gradientClass} opacity-40 group-hover:opacity-60 transition-opacity duration-700`}></div>
        ...
        <div className="absolute bottom-0 left-0 p-5 w-full z-10">
             <div className="flex items-end gap-2 mb-1">
                <h3 className="text-3xl font-bold text-white leading-tight">{candidate.name}</h3>
                <span className="text-zinc-400 text-xl font-medium mb-1">{candidate.age}</span>
             </div>
            <p className="text-zinc-300 text-sm line-clamp-2 opacity-90 font-light">{candidate.bio}</p>
        </div>
      </div>
        ...
        <div className="mt-auto pt-4 border-t border-white/5">
             <div className="flex flex-wrap gap-x-3 gap-y-1">
                {candidate.interests.slice(0,4).map((int, i) => (
                     <span key={i} className="text-xs text-zinc-500 font-medium hover:text-zinc-300 transition-colors">#{int.toLowerCase()}</span>
                ))}
             </div>
        </div>
      </div>
